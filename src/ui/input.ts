 ...
 * desktop: the right button always pans whatever tool is held, a right click
 * with no movement cancels, a plain two-finger scroll pans while a wheel
 * notch or a pinch zooms, and the zoom eases toward its target rather than
 * jumping a notch at a time.
 *
 * The valley is a real place rather than a picture of one, so it can also be
 * turned: the middle button drags the view round and tilts it, two fingers
 * twist, and `Q` and `E` turn a step at a time. Zoom keeps the wheel, the
 * pinch, `[` and `]`, and the edge of the view.
 */
import { Camera, MAX_ZOOM, MIN_ZOOM, ORBIT_RADIANS_PER_PIXEL } from '../render/camera';
...
/** Turn per second while a turn key is held, in CSS pixels of orbit drag. */
const KEY_TURN_RATE = 190;
/** Twist below this is the wobble in an ordinary pinch, not a turn. */
const TWIST_DEADZONE = 0.02;
...
/**
 * Buttons that take the camera rather than the tool, whatever is held. The
 * right one pans, as it did when the map was flat; the middle one turns.
 */
...
export class InputController {
  ...
  /** Angle between them, so a twist turns the valley. */
  ...
  /**
   * `Q` and `E` turn the valley. They zoomed when there was nothing to turn;
   * now that there is, turning is what they are worth, and zoom keeps the
   * wheel, the pinch, the brackets and the edge of the view.
   */
  private applyKeyTurn(dt: number): void {
    if (this.keys.size === 0) return;
    const left = this.keys.has('q') ? 1 : 0;
    const right = this.keys.has('e') ? 1 : 0;
    if (left === right) return;
    this.camera.orbitByScreen((right - left) * KEY_TURN_RATE * dt, 0);
  }

  private applyKeyZoom(dt: number): void {
    if (this.keys.size === 0) return;
    const inward = this.keys.has(']') ? 1 : 0;
    const outward = this.keys.has('[') ? 1 : 0;
  ...
  private updateCursor(): void {
    const next =
      this.panning || this.orbiting ? 'grabbing' : this.handlers.isDrawing() ? 'crosshair' : 'grab';
  ...
  /** A pointer that takes the camera rather than the tool, whatever is held. */
  private static movesCamera(pointer: ActivePointer): boolean {
    return pointer.button === MIDDLE_BUTTON || pointer.button === RIGHT_BUTTON;
  }

  /** A pointer that turns the view rather than sliding it. */
  private static orbits(pointer: ActivePointer): boolean {
    return pointer.button === MIDDLE_BUTTON;
  }
  ...
  private onPointerMove = (event: PointerEvent): void => {
    ...
    if (InputController.orbits(pointer)) {
      this.orbiting = true;
      this.camera.orbitByScreen(point.x - previousX, point.y - previousY);
    } else if (this.handlers.isDrawing() && !InputController.movesCamera(pointer)) {
      if (!this.dragging) {
        this.dragging = true;
  ...
  private updatePinch(): void {
    ...
    // And a twist turns the valley. Converted into the pixels an orbit drag
    // would have taken, so a turn feels the same under two fingers as under
    // the middle button. Small twists are ignored: every pinch has a little
    // rotation in it, and the horizon should not drift while zooming.
    const angle = Math.atan2(b.y - a.y, b.x - a.x);
    let turn = angle - this.pinchAngle;
    while (turn > Math.PI) turn -= Math.PI * 2;
    while (turn < -Math.PI) turn += Math.PI * 2;
    if (Math.abs(turn) > TWIST_DEADZONE) this.camera.orbitByScreen(turn / ORBIT_RADIANS_PER_PIXEL, 0);
